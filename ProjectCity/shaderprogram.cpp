...
bool ShaderProgram::load(const std::string &vertexshaderfile, const std::string &fragmentshaderfile)
{
	...
	// At this point we need to bind attribute locations if we want to define them.
	// There are couple of reasons why to do that instead of querying them with glGetAttribLocation():
	// 1) If you use the same VAO data with different shaders, each shader might expect them to be set in different positions!
	// 2) You force shader compilation to be synchronous by querying those values sequentially that will slow things down.
	// "It is also permissible to bind a generic attribute index to an attribute variable name that is never used in a vertex shader." - OpenGL documentation
	// https://www.opengl.org/sdk/docs/man/html/glBindAttribLocation.xhtml
	// We define used attribute locations in shaderprogram.h methods.
	/*glBindAttribLocation(shaderprogram, getPositionAttribLocation(), "in_Position");
	glBindAttribLocation(shaderprogram, getColorAttribLocation(), "in_Color");
	glBindAttribLocation(shaderprogram, getNormalAttribLocation(), "in_Normal");
	glBindAttribLocation(shaderprogram, getTexture0AttribLocation(), "in_TexCoord0");
	glBindAttribLocation(shaderprogram, getTexture1AttribLocation(), "in_TexCoord1");*/
